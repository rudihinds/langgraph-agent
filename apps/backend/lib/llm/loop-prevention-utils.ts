 ...
 * @returns A node function that can be added to a StateGraph
 */
function createProgressDetectionNode<
  T extends WithLoopPrevention & Record<string, any>,
>(
  ...
  return function progressDetectionNode(
    state: T
  ): T | { next: string; reason?: string } {
    ...
    const noProgressDetected =
      iterationsWithoutProgress >= maxNoProgressIterations;
...
function createIterationLimitNode<T extends WithLoopPrevention>(
  ...
  return function iterationLimitNode(
    state: T
  ): T | { next: string; reason?: string } {
    // Initialize loop prevention if not present
    if (!state.loopPrevention) {
...
function createCompletionCheckNode<T extends Record<string, any>>(
  ...
  return function completionCheckNode(
    state: T
  ): T | { next: string; reason?: string } {
 ...
 * @returns A node function that can be added to a StateGraph
 */
function createSafetyCheckNode<
  T extends WithLoopPrevention & Record<string, any>,
>(
  options: {
    maxIterations?: number;
  ...
  return function safetyCheckNode(
    state: T
  ): T | { next: string; reason?: string } {
    // Initialize loop prevention if not present
    if (!state.loopPrevention) {
