...
import { LLMCompletionOptions, LLMStreamEvent, LLMStreamEventType } from "../types.js";
...
export function createStreamingNode<T = any>(
  ...
  return async (): Promise<StreamingNodeResult<T>> => {
    ...
    streamManager.streamCompletion(options, (event: LLMStreamEvent) => {
      switch (event.type) {
        case LLMStreamEventType.Content:
          ...
          if (debug) {
            logger.debug(`[StreamingNode:${channelId}] Content: ${event.content}`);
          }
      ...
      if (debug) {
        logger.debug(`[StreamingNode:${channelId}] Stream failed: ${error.message}`);
      }
