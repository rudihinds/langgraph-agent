...
export const OverallProposalStateSchema = z.object({
  ...
  // Section processing - Fixed to use Record instead of Map
  sections: z.record(
    z.string(),
    z.object({
      id: z.string(),
  ...
  funderIntelligence: z.object({
    organizationalPriorities: z.array(
      z.object({
        ...
        confidence: z.number(), // Added missing confidence field
      })
    ),
...
// Industry Analysis schema - New schema to match TypeScript interface
...
// Competitive Analysis schema - New schema to match TypeScript interface
...
// Requirement Analysis schema - New schema to match TypeScript interface
...
// Evaluation Criteria schema - New schema to match TypeScript interface
...
// Strategy Decision schema - New schema to match TypeScript interface
...
// Solution Spec schema - New schema to match TypeScript interface
...
// User Collaboration schema - Updated to match TypeScript interface
...
// Adaptive Workflow schema - Updated to match TypeScript interface
...
// Full Planning Intelligence schema - Updated to match TypeScript interface exactly
