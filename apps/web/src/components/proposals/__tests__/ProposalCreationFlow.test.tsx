...
vi.mock('../FunderDetailsView', () => ({
  default: ({ onSubmit }: any) => (
    <div data-testid="funder-details-view">
      <button onClick={() => onSubmit({ funderName: 'Test Foundation' })}>Submit Funder Details</button>
    </div>
  ),
}));
...
vi.mock('../ApplicationQuestionsView', () => ({
  default: ({ onSubmit }: any) => (
    <div data-testid="application-questions-view">
      <button onClick={() => onSubmit({ questions: [{ question: 'Test Question' }] })}>Submit Questions</button>
    </div>
  ),
}));
...
vi.mock('../RFPResponseView', () => ({
  default: ({ onSubmit }: any) => (
    <div data-testid="rfp-response-view">
      <button onClick={() => onSubmit({ document: { name: 'test.pdf' } })}>Submit RFP</button>
    </div>
  ),
}));
...
describe('ProposalCreationFlow', () => {
  ...
  it('renders the first step (FunderDetailsView) initially', () => {
    render(<ProposalCreationFlow proposalType="application" onCancel={mockOnCancel} />);
  ...
  it('navigates through the application proposal flow', async () => {
    ...
    expect(screen.getByTestId('application-questions-view')).toBeInTheDocument();
