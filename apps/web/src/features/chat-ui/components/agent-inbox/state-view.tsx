    ...
    <div className="relative flex flex-row items-start justify-start gap-2 text-sm">
      ...
      <div className="flex w-full flex-col items-start justify-start gap-1">
        ...
        <motion.div
          ...
          className="w-full"
        >
          <StateViewRecursive value={props.value} expanded={expanded} />
        </motion.div>
      </div>
    </div>
  );
...
export function StateView(props: {
  ...
  return (
    <div className="overflow-hidden rounded-md border border-gray-200 bg-white px-4 py-3 text-sm shadow-sm">
      <div className="flex flex-row items-center justify-between pb-2">
        <div className="flex flex-row items-center gap-2">
          <h2 className="text-lg font-semibold">State</h2>
          <Button
            className="flex flex-row items-center justify-center gap-1 px-2 py-1 h-8"
            variant="outline"
      ...
      {props.isLoading ? (
        ...
        <div className="max-h-[70vh] overflow-auto pb-2">
          <div className="relative ml-6 flex w-full flex-col items-start gap-1">
            {/* Vertical line */}
            <div className="absolute top-0 left-[-24px] h-full w-[1px] bg-gray-200" />

            {Object.entries(props.stateObject).map(([key, value], idx) => (
              <div
                key={`state-view-object-${key}-${idx}`}
                className="relative w-full"
              >
