...
jest.mock('@/components/proposals/ServerForm', () => ({
  __esModule: true,
...
describe('ReviewProposalView', () => {
  ...
  it('prepares form data with dates in ISO format for API submission', () => {
    ...
    expect(formData.metadata.funder_details.deadline).toEqual(expect.stringContaining('T'));
  });
  ...
  it('handles RFP proposal type correctly', () => {
    ...
    expect(formData.metadata.rfp_details.rfpUrl).toBe('https://example.com/rfp');
