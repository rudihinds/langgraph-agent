...
jest.mock('@/components/ui/enhanced-appointment-picker', () => ({
...
describe('FunderDetailsView', () => {
  ...
  it('loads data from localStorage on mount if available', async () => {
    ...
    await act(async () => {
      render(
        <FunderDetailsView onSubmit={mockOnSubmit} onBack={mockOnBack} />
      );
    });
  ...
  it('saves data to localStorage when form data changes', async () => {
    ...
    // Update a field
    const orgNameInput = screen.getByPlaceholderText(/Enter the name of the funding organization/i);
  ...
  it('calls onSubmit with correct data format when form is submitted', async () => {
    ...
    // Fill out the form
    await act(async () => {
      fireEvent.change(screen.getByPlaceholderText(/Enter the name of the funding organization/i), { 
        target: { value: 'Test Organization' } 
      });
      
      fireEvent.change(screen.getByPlaceholderText(/Enter the title of the grant/i), { 
      ...
      fireEvent.change(screen.getByPlaceholderText(/e.g., Education, Healthcare/i), { 
  ...
  it('validates the form and shows error messages when invalid', async () => {
    ...
    expect(screen.getByText(/Grant\/Funding Title is required/i)).toBeInTheDocument();
